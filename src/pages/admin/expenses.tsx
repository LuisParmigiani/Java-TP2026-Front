...
import { CreditCard, Wallet, History, Plus, Truck, Users, Zap, MoreHorizontal } from "lucide-react";
...
import type { CamionResponse, PersonaResponse, GastoRequest, GastoResponse } from "../../services/Interfaces";
import { getAll, postExpense } from "../../services/ExpensesService";
import Pagination from "../../components/Pagination";

type TabType = "camion" | "sueldo" | "servicios" | "otros";

const SERVICIOS = ["Luz", "Gas", "Agua", "Internet", "Alquiler", "Teléfono", "Seguro", "Mantenimiento"];
...
const TABS: { key: TabType; label: string; icon: React.ReactNode }[] = [
    ...
    { key: "otros", label: "Otros", icon: <MoreHorizontal className="w-4 h-4" /> },
];

function gastoIcon(g: GastoResponse) {
    if (g.camionId) return <Truck className="w-4 h-4 text-primary" />;
    if (g.detalle?.startsWith("Sueldo -")) return <Users className="w-4 h-4 text-primary" />;
...
export default function Expenses() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const personasData = await fetchPersonas(0, 1000000000);
    ...
    const isFormValid = () => {
        ...
        switch (activeTab) {
            case "camion": return !!selectedTruckId;
            case "sueldo": return !!selectedEmpleado;
            case "servicios": return !!selectedServicio;
            case "otros": return !!otrosDetalle.trim();
        }
    };
    ...
    const handleSubmit = async () => {
        ...
        switch (activeTab) {
            case "camion":
                gasto = { ...base, detalle: camionDetalle, camion_id: parseInt(selectedTruckId) };
                break;
            case "sueldo":
        ...
        <div className="min-h-screen bg-background text-foreground flex flex-col">
            ...
            <main className="flex-grow container mx-auto px-4 py-8 max-w-4xl">
                <div className="mb-8">
                    ...
                    <p className="text-gray-500 mt-1">Registrá los gastos de la empresa.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-semibold text-gray-800">Nuevo gasto</h2>
                        </div>
                        ...
                        {activeTab === "camion" && (
                            <div className="flex flex-col gap-3">
                                <div>
                                    <label className="block text-sm font-medium text-gray-600 mb-1">Camión</label>
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        value={camionDetalle}
                                        onChange={(v) => typeof v === "string" && setCamionDetalle(v)}
                                        color="primary"
                                        size="full"
                                    />
                                </div>
                        ...
                        {activeTab === "sueldo" && (
                            <div>
                                <label className="block text-sm font-medium text-gray-600 mb-1">Empleado</label>
                                <Select value={selectedEmpleado} onValueChange={setSelectedEmpleado}>
                                    ...
                                    <SelectContent>
                                        {personas.map((p) => (
                                            <SelectItem key={p.id} value={`${p.nombre} ${p.apellido}`}>
                                                {p.nombre} {p.apellido}
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                ...
                                {gastos.map((g) => (
                                    <div key={g.id} className="flex items-center justify-between py-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="text-sm font-medium text-gray-800">{g.detalle}</p>
