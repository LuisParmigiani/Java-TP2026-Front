        ...
        <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            {alert && (
                <div className="px-4 pt-4 max-w-4xl mx-auto w-full">
                    ...
                    >
                        <AlertTitle>
                            {alert === 'success' ? '¡Saldo cargado con éxito!' : '¡Error al cargar el saldo!'}
                        </AlertTitle>
                        <AlertDescription>
                            {alert === 'success'
            ...
            <main className="flex-grow container mx-auto px-4 py-8 max-w-4xl">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-500 mt-1">Recargá tu cuenta para pagar tus pedidos fácilmente.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <InformationCard
                        ...
                        title={fetchLoading ? "..." : formatCurrency(saldo)}
                        description="Usá tu saldo para pagar pedidos"
                        cardColor="white"
                        titleColor={saldo < 0 ? "red" : "primary"}
                        descriptionColor="gray-500"
                        size="md"
                        svg={<Wallet className="w-5 h-5 text-primary" />}
                    />

                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col gap-4">
                        <div className="flex items-center gap-2">
                            <CreditCard className="w-5 h-5 text-primary" />
                            <h2 className="text-lg font-semibold text-gray-800">Nueva recarga</h2>
                        </div>
                            ...
                            <>
                                <div>
                                    ...
                                    <Input
                                        ...
                                        size="full"
                                    />
                                    <p className="text-xs text-gray-400 mt-1">PNG, JPG o PDF. Máx 10MB.</p>
                                </div>
                                <Button
                                    variant="primary"
                                    size="full"
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    {fetchLoading ? (
                        ...
                        <div className="text-center py-10">
                            ...
                            <p className="text-gray-400 text-sm">No tenés recargas registradas todavía.</p>
                        </div>
                    ) : (
                        <div className="divide-y divide-gray-50">
                            {pagos.map((pago) => (
                                <div key={pago.id} className="flex items-center justify-between py-3 hover:bg-gray-50/60 -mx-2 px-2 rounded-lg transition-colors duration-150">
                                    <div className="flex items-center gap-3">
                                        <div className="w-9 h-9 bg-gray-50 rounded-full flex items-center justify-center border border-gray-100">
                                            {METODO_ICONS[pago.metodoPago] ?? <Banknote className="w-4 h-4 text-gray-400" />}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium text-gray-800">{pago.metodoPago}</span>
                                            <span className="text-xs text-gray-400">{formatDate(pago.fecha)}</span>
                                        </div>
                                    </div>
