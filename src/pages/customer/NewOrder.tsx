    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      {showAlert && (
        <div className="fixed inset-0 flex items-center shadow-lg justify-center z-50 bg-transparent backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
            ...
            <p className="mb-6">
              ¿Estás seguro que deseas cambiar la dirección? Esto reseteará los
      ...
      <NewOrderCard
        ...
        direction={directions.find(
          (dir) => String(dir.id) == selectedDirection,
        )}
        products={products}
        total={total}
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        <div className="rounded-2xl  p-6 md:p-8 ">
          ...
          <p className="text-gray-600 mt-2 mb-6">
            Crea un nuevo pedido seleccionando los productos que deseas comprar.
          </p>
          {directions.length > 0 ? (
            <div className="mb-6">
              <label
            ...
            <div className="mb-6 rounded-md bg-yellow-50 p-4">
              <p className="text-sm text-yellow-700">
                No se encontraron direcciones de entrega. Por favor, agregue una
                dirección en su perfil para continuar con su pedido.
              </p>
            </div>
          )}
          {selectedDirection === "" && directions.length > 0 ? (
            <div className="mb-6 rounded-md bg-red-50 p-4">
              <p className="text-sm text-red-700">
                Por favor, seleccione una dirección de entrega para continuar
                con su pedido.
              </p>
            </div>
          ) : (
            directions.length > 0 && (
              <>
                ...
                <ProductsFilter
                  ...
                  setAppliedMinPrice={(v) => {
                    setAppliedMinPrice(v);
                    setPage(1);
                  }}
                  setAppliedMaxPrice={(v) => {
                    setAppliedMaxPrice(v);
                    setPage(1);
                  }}
