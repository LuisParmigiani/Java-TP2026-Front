...
export default function SemanalOrder() {
    ...
    const description = (id: number) => () => {
        ...
        if (direction) {
            if (direction.pedidosSemanales && Array.isArray(direction.pedidosSemanales)) {
                const nuevasLineas = direction.pedidosSemanales
                    .map((linea) => {
                        const subtotal = linea.cantidad * linea.productoZona.producto.precio;
                        if (linea.cantidad === 0) {
                    ...
                    >
                        ...
                        >
                            <div className="mb-6 flex items-start justify-between gap-4">
                                <div>
                                    <h2 className="text-2xl font-semibold text-gray-900">Detalle del pedido semanal</h2>
                                    <p>Direccion: {directionSelected?.calle}, Número: {directionSelected?.numero}  {directionSelected?.casa ? `Casa: ${directionSelected.casa}` : ''}</p>
                                </div>
                                <button
                                    onClick={() => setShowDescription(false)}
                            ...
                            <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50/60 p-4">
                                ...
                                <ProductTable lineasPedido={lineasPedido} total={lineasPedido.filter((linea): linea is NonNullable<typeof linea> => linea !== null).reduce((acc, linea) => acc + linea.subtotal, 0)} />
                                <div className="mx-auto mt-4 w-full max-w-3xl text-center text-sm text-gray-500">
                                    <Button
                                        color="primary"
                                        size="md"
                                        onClick={() => navigate('/customer/editWeeklyOrder', { state: { domicilioId: directionSelected?.id } })}
                                    >
                        ...
                        <div key={direction.id} className="h-full">
                            <div className="bg-white border border-gray-200 rounded-xl p-5 flex flex-col h-full transition-colors hover:border-primary/60">
                                ...
                                <div className="mb-3 flex-1">
                                    ...
                                    <p className="text-sm mt-1 text-gray-700">Calle: {direction.calle}, Número: {direction.numero}  {direction?.casa ? `Casa: ${direction.casa}` : ''}</p>
                                </div>
                                <Button
                                    variant="ghost"
