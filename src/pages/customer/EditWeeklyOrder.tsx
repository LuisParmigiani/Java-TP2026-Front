...
export default function EditWeeklyOrder() {
    ...
    const [initialProducts, setInitialProducts] = useState<{ product: ProductoResponse, quantity: number }[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await getById(domicilioId, ['pedidoSemanal', 'productoZona', 'producto']);
                if (response) {
                    if (response.pedidosSemanales && Array.isArray(response.pedidosSemanales)) {
                        const lineasDesdeServidor = response.pedidosSemanales.map((linea) => ({
        ...
        fetchData();
    }, [domicilioId, token, appliedSearchTerm, appliedMinPrice, appliedMaxPrice, appliedDirection, sortOption]);
    ...
    const totalAmount = tableLinesData.reduce((acc, linea) => acc + linea.subtotal, 0);

    // Detecta si hay cambios comparando con el estado inicial
    const hasChanges = selectedProducts.length !== initialProducts.length ||
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50  border-gray-200 bg-gray-50/60">
                    <div className="rounded-xl  p-4">
                        <Alert variant="success" >
                            <AlertTitle>¡El pedido semanal se ha modificado con exito.!</AlertTitle>
                            <AlertDescription>
                                <div className="flex flex-col items-center gap-2">
                                    Muchas gracias por modificar el pedido semanal.
                                    <LinkButton url="/customer/semanalOrder" variant="outline" size="sm" name='Finalizar.' />
                                </div>
                            </AlertDescription>
                        </Alert>
                    </div>
            ...
            {alert === -1 &&
                <div>
                    ...
                    <Alert variant="danger" autoClose={true} onClose={() => setAlert(0)}>
                        <AlertTitle>¡El pedido semanal no se ha podido modificar.!</AlertTitle>
