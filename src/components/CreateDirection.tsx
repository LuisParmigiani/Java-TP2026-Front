    ...
    const loadData = async () => {
      try {
        const [zonaResponse, personResponse] = await Promise.all([
          fetchZones(token),
          // Aprovechamos el nivelAcceso="Usuario" desde el back que agregamos hace rato:
          fetchPersonas(token, 0, 1000000000, ["usuario"]),
        ]);
        setZonas(zonaResponse);
        // Además, agregamos el signo de pregunta '?' (optional chaining) por si algún cliente no tiene usuario asignado:
