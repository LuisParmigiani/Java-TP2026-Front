...
export default function InformationCard(prop: Prop) {
    return (
        <div className={`w-full h-full justify-center ${cardColors[prop.cardColor]}  flex flex-col gap-2 rounded-lg p-4 size-${size[prop.size]} border-1 border-${cardColors[prop.cardColor]} shadow-sm`}>
            ...
            <h3 className={`text-4xl font-bold m-3 ${titleColors[prop.titleColor]}`}>{prop.title}</h3>
            <p className={`text-sm mt-2 ${descriptionColors[prop.descriptionColor]}`}>{prop.description}</p>
        </div>
    );
}