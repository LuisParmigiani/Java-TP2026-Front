...
const SelectTrigger = React.forwardRef<HTMLButtonElement, React.HTMLAttributes<HTMLButtonElement>>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'group flex h-9 w-full items-center justify-between whitespace-nowrap rounded-md border-2 border-primary/80 bg-white ',
        'px-3 py-2 text-sm shadow-sm ring-offset-background focus:outline-none',
        'hover:bg-primary/20  hover:border-primary transition-colors',
        'focus:ring-2 focus:ring-primary/50 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
        className,
      )}
      {...props}
    >
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          ' rounded-md border bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out',
          ' data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
          ' data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 ',
          'data-[side=top]:slide-in-from-bottom-2 origin-[--radix-select-content-transform-origin] bg-white',
          position === 'popper' &&
